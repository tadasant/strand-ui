...
from app.slack_integration.models import SlackAgent, SlackTeam, SlackChannel, SlackEvent, SlackUser
    ...
    for user_info in users_info:
        ...
        SlackUser.objects.get_or_create(id=user_info.get('id'),
                                        defaults=dict(name=user_info.get('name'),
                                                      first_name=user_info['profile']
                                                      .get('first_name', ''),
                                                      last_name=user_info['profile']
                                                      .get('last_name', ''),
                                                      real_name=user_info.get('real_name', ''),
                                                      display_name=user_info['profile']
                                                      .get('display_name'),
                                                      email=user_info['profile'].get('email'),
                                                      avatar_72=user_info['profile']
                                                      .get('image_72'),
...
def create_topics_and_discussions(client, slack_team, bot_id):
    ...
    for channel_info in filtered_channels_info:
        ...
        if is_valid_channel_history(channel_history['messages']):
            ...
            temp_original_poster = User.objects.get(slack_users__id=bot_id)
            ...
            topic = Topic.objects.create(title=topic_info['title'], description=topic_info['description'],
                                         original_poster=temp_original_poster, group=slack_team.slack_agent.group)
            ...
            discussion.participants.set(User.objects.filter(slack_users__id__in=participants))

            slack_channel, _ = SlackChannel.objects.get_or_create(id=channel['id'],
                                                                  defaults=dict(name=channel['name'],
                                                                                slack_team=slack_team,
                                                                                discussion=discussion))

            for message in messages:
                author = User.objects.get(slack_users__id=message['slack_user_id'])
            ...
            for reply in replies:
                author = User.objects.get(slack_users__id=reply['slack_user_id'])
